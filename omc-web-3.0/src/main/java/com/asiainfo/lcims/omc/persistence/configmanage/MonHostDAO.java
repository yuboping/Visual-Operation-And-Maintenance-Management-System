...
public interface MonHostDAO {
    ...
    @SelectProvider(method = "getHost", type = SqlProvider.class)
    List<MonHost> getHost(@Param("hostname") String hostname, @Param("addr") String addr,
            @Param("hosttype") String hosttype, @Param("nodeid") String nodeid);

    // 新增主机
    @InsertProvider(method = "insertMonHost", type = MonHostDAOImpl.class)
    int insert(@Param("host") MonHost host);

    // 修改主机
    @UpdateProvider(method = "updateMonHost", type = MonHostDAOImpl.class)
