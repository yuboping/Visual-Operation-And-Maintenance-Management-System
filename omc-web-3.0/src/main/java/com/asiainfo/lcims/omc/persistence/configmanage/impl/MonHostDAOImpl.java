...
public class MonHostDAOImpl {
    ...
    public String updateMonHost(Map<String, Object> paras) {
        ...
        if (null != monHost.getHosttype()) {
            buffer.append(",HOSTTYPE='").append(monHost.getHosttype()).append("' ");
        }
    ...
    public String insertMonHost(Map<String, Object> paras) {
        ...
        StringBuffer buffer = new StringBuffer(
                "INSERT INTO MON_HOST(HOSTID,ADDR,HOSTNAME,NODEID,STATUS,OS,PRODUCTNAME,HOSTTYPE,CPU,MEMORY,DISKSPACE,HOST_ROOM,RACK_NUM,SERIALNUMBER,IPV4,IPV6,LOCATION) VALUES('");
        ...
        buffer.append(host.getNodeid()).append("',").append(host.getStatus()).append(",'")
                .append(host.getOs()).append("','").append(host.getProductname()).append("','")
                .append(host.getHosttype()).append("','").append(host.getCpu()).append("','")
                .append(host.getMemory()).append("','").append(host.getDiskspace()).append("','")
                .append(host.getHost_room()).append("','").append(host.getRack_num()).append("','")
                .append(host.getSerialnumber()).append("','").append(host.getIpv4()).append("','")
                .append(host.getIpv6()).append("','").append(host.getLocation()).append("')");
