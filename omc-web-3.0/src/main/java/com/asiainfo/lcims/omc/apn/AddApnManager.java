...
public class AddApnManager implements InitializingBean {

    private static final Logger LOG = LoggerFactory.getLogger(AddApnManager.class);
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        try {
            ...
            TriggerKey triggerKey = TriggerKey.triggerKey(QuartzConstant.TIME_ADD_APN_TRIGGER_KEY);
            ...
            if (null == trigger) {// 不存在，创建一个
                // 创建执行类
                JobDetail jobDetail = JobBuilder.newJob(AddApnJob.class)
                        .withIdentity(QuartzConstant.TIME_ADD_APN_TRIGGER_KEY).build();
                jobDetail.getJobDataMap().put("mdApnRecordDAO", mdApnRecordDAO);
                jobDetail.getJobDataMap().put("commoninit", commoninit);
                // 创建定时触发器
                trigger = TriggerBuilder.newTrigger()
                        .withIdentity(QuartzConstant.TIME_ADD_APN_TRIGGER_KEY)
            ...
            scheduler.start();
            LOG.info("-------------- add apn trigger is running.--------------");
        } catch (Exception e) {
