...
require(['jquery', 'layer', 'daterangepicker', 'laypage', 'resizewh' ],
	function($, layer, daterangepicker,laypage,resizewh) {
	    resizewh.resizeWH($("#mainqryRadiusHostAuth"));
	    butBindFunction();
	    showDayTime();
...
//-----------------------------------------以下为自定义方法-------------------------------------------------//            
	    ...
	    function loadOptRecord(){
	        ...
	        $.getJSON("/view/class/reporttool/getRadiusHostAuth?random=" + Math.random(),data, function(result){
	        	console.log(result);
	    ...
	    function showTable(data,startnum,endnum){
            ...
            if(data==null||data.length==0){
            	...
            	$("#qryRadiusHostAuth").empty().append(rowdata);
            }else{
            	for(var i=startnum;i<=endnum;i++){
                    ...
                    +rowninfo[7]+"</td><td>"+rowninfo[8]+"</td>"
                    +"</tr>";
                }
            	$("#qryRadiusHostAuth").empty().append(rowdata);
            }
	    ...
	    function exportExport(){
	        ...
	        var reportId = $("#reportId").val();
			window.open("/view/class/reporttool/exportRadiusHostAuth?querydate="+querydate+"&querytype="+querytype+"&reportId="+reportId+"&random=" + Math.random(),"_blank");
		}
});