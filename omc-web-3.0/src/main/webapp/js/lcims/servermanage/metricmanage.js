...
//----------------------------------以下为自定义方法-------------------------------------------------//
    ...
    function addShow(){
    	reset("add_div");
        $.getJSON("/view/class/system/metricmanage/query/mdcollcyclelist?random=" + Math.random(), function(result) {
            $("#add_cycle_id").empty();
            $.each(result,function(i,data){
                $("#add_cycle_id").append("<option value=\""+data.cycleid+"\">"+data.cyclename+"</option>");
            });
        });
        $.getJSON("/view/class/system/metricmanage/query/mdparamlist?type=7&random=" + Math.random(), function(result) {
            $("#add_script_return_type").empty();
            $.each(result,function(i,data){
                $("#add_script_return_type").append("<option value=\""+data.code+"\">"+data.description+"</option>");
            });
        });
        $.getJSON("/view/class/system/metricmanage/query/mdmetrictypelist?random=" + Math.random(), function(result) {
            $("#add_metric_type").empty();
            $.each(result,function(i,data){
            	$("#add_metric_type").append("<option value=\""+data.id+"\">"+data.metric_type_name+"</option>");
            });
        });
        $("#add_server_type").empty();
        $("#add_server_type").append("<option value=\"\">请选择</option>");
        $.getJSON("/view/class/system/metricmanage/query/servertypelist?random=" + Math.random(), function(result) {
            $.each(result,function(i,data){
            	$("#add_server_type").append("<option value=\""+data.code+"\">"+data.description+"</option>");
            });
        });
        
      showLayer("add_div",'新增指标');
    }
    ...
    function modifyShow(){
    	...
    	if(checkboxArray.length != 1){
    		layer.msg("修改请只选择一个指标!",{
                time:2000,
                skin: 'layer_msg_color_alert'
            });
    	}else{
    		var metricid = checkboxArray[0];
            reset("modify_div");
            
            loadingwait();
            var data = {id : metricid};
            $.ajax({
                type: "post",
                url: "/view/class/system/metricmanage/query?random=" + Math.random(),
                data: data,
                cache: false,
                async: false, 
                dataType: "json",
                success: function (result) {
                	if(result.length>0){
                        var metric = result[0];
                        $("#modify_metric_identity").val(metric.metric_identity);
                        $("#modify_metric_name").val(metric.metric_name);
                        $("#modify_script").val(metric.script);
                        $("#modify_script_param").val(metric.script_param);
                        $("#modify_description").val(metric.description);
                        $.getJSON("/view/class/system/metricmanage/query/mdcollcyclelist?random=" + Math.random(), function(result) {
                            $("#modify_cycle_id").empty();
                            $.each(result,function(i,data){
                                $("#modify_cycle_id").append("<option value=\""+data.cycleid+"\">"+data.cyclename+"</option>");
                            });
                            $("#modify_cycle_id").val(metric.cycle_id);
                        });
                        $.getJSON("/view/class/system/metricmanage/query/mdparamlist?type=7&random=" + Math.random(), function(result) {
                            ...
                            $("#modify_script_return_type").val(metric.script_return_type);
                        });
                        
                        
                        $.getJSON("/view/class/system/metricmanage/query/mdmetrictypelist?random=" + Math.random(), function(result) {
                        	$("#modify_metric_type").empty();
                        	$.each(result,function(i,data){
                            	$("#modify_metric_type").append("<option value=\""+data.id+"\">"+data.metric_type_name+"</option>");
                            });
                            $("#modify_metric_type").val(metric.metric_type); 
                        });
                        ...
                        $.getJSON("/view/class/system/metricmanage/query/servertypelist?random=" + Math.random(), function(result) {
                            var server=""
                            $.each(result,function(i,data){
                            	if(data.description==metric.server_type){
                            		server=data.code
                            	}
  ...
  //详情按钮事件
    function detailShow(metricid){
        reset("detail_div");
        loadingwait();
        $.ajax({
            type: "post",
            url: "/view/class/system/metricmanage/query?id="+metricid+"&random=" + Math.random(),
            cache: false,
            async: false, 
            success: function (result) {
            	if(result.length>0){
                    var metric = result[0];
                    $("#detail_metric_identity").val(metric.metric_identity);
                    $("#detail_metric_name").val(metric.metric_name);
                    $("#detail_cycle_id").val(metric.cyclename);
                    $("#detail_script").val(metric.script);
                    $("#detail_script_param").val(metric.script_param);
                    $("#detail_script_return_type").val(metric.metric_type_name);
                    $("#detail_metric_type").val(metric.mdparamdescription);
                    $("#detail_description").val(metric.description);
                    $("#detail_server_type").val(metric.server_type);
                    showLayerDetail("detail_div",'指标详情');
                }
            }
         });
    }
    ...
    function modifyInfo(){
    	...
    	if(checkboxArray.length != 1){
    		...
    		var metricid = checkboxArray[0];
        	var metric_identity = stringutil.Trim($("#modify_metric_identity").val());
            var metric_name = stringutil.Trim($("#modify_metric_name").val());
            ...
            var metric_type = $("#modify_metric_type").val(); 
            var server_type = $("#modify_server_type").val(); 
            var description = stringutil.Trim($("#modify_description").val());
            if(stringutil.checkString("modify_metric_identity",metric_identity,"指标标识不能为空!")|| 
            		stringutil.checkString('modify_metric_identity',metric_identity,"指标标识不能超过30位!",30) ||
            		stringutil.checkString('modify_metric_name',metric_name,"指标名称不能为空!") || 
            		stringutil.checkString('modify_metric_name',metric_name,"指标名称不能超过50位!",50) ||
            		...
            		stringutil.checkString('modify_metric_type',metric_type,"指标类型不能为空!")||
            		stringutil.checkString('modify_description',description,"指标描述不能超过100位!",100)){
                return;
            }
            var state = true;
            //校验指标标识是否重复事件
            $.ajax({
                ...
                cache: false,
                async: false, 
                data: {metric_identity:metric_identity},
                success: function (result) {
                	var length = result.length;
                	for (var i=0;i<length;i++){
                		if(result[i].metric_identity==metric_identity){
                			if(result[i].id != metricid){
                				state = false;
                				$("#modify_metric_identity").focus();
                                layer.tips("指标标识不能重复", '#modify_metric_identity',{ tips: [2, '#EE1A23'] });
                        		return;
                			}
            			}
            		}
            ...
            if(state){
        		loadingwait();
                var data= {id:metricid,metric_identity:metric_identity,metric_name:metric_name,cycle_id:cycle_id,script:script,script_param:script_param,script_return_type:script_return_type,metric_type:metric_type,description:description,server_type:server_type};
                $.getJSON("/view/class/system/metricmanage/modify?random=" + Math.random(),data, function(result) {
                    layer.closeAll();
    ...
    //新增确认
    function addInfo(){
        var metric_identity = stringutil.Trim($("#add_metric_identity").val());
        var metric_name = stringutil.Trim($("#add_metric_name").val());
        ...
        var metric_type = $("#add_metric_type").val(); 
        var server_type = $("#add_server_type").val();
        if(server_type == null ){
        	server_type = 'null';
        }
        var description = stringutil.Trim($("#add_description").val());
        if(stringutil.checkString("add_metric_identity",metric_identity,"指标标识不能为空!")|| 
        		stringutil.checkString('add_metric_identity',metric_identity,"指标标识不能超过30位!",30) ||
        		stringutil.checkString('add_metric_name',metric_name,"指标名称不能为空!") || 
        		stringutil.checkString('add_metric_name',metric_name,"指标名称不能超过50位!",50) ||
        		...
        		stringutil.checkString('add_metric_type',metric_type,"指标类型不能为空!")||
        		stringutil.checkString('add_description',description,"指标描述不能超过100位!",100)){
            return;
        }
        //校验指标标识是否重复事件
        $.ajax({
            ...
            success: function (result) {
            	var length = result.length;
            	if(length > 0){
            		$("#add_metric_identity").focus();
                    layer.tips("指标标识不能重复", '#add_metric_identity',{ tips: [2, '#EE1A23'] });
            		return;
            	}else{
            		loadingwait();
                    var data= {metric_identity:metric_identity,metric_name:metric_name,cycle_id:cycle_id,script:script,script_param:script_param,script_return_type:script_return_type,metric_type:metric_type,description:description,server_type:server_type};
                    $.getJSON("/view/class/system/metricmanage/add?random=" + Math.random(),data, function(result) {
                        layer.closeAll();
    ...
    function loadOptRecord(){
        ...
        var metric_name = stringutil.Trim($("#metric_name").val());
        var metric_type = stringutil.Trim($("#metric_type").val());
        var data = {'metric_name':metric_name,'metric_type':metric_type};
        $.getJSON("/view/class/system/metricmanage/query?random=" + Math.random(),data, function(result) {
    ...
    function showTable(data,startnum,endnum){
        ...
        for(var i=startnum;i<=endnum;i++){
            ...
            + "<td class='over_ellipsis' style='max-width:70px' title=\""+stringutil.isNull(rowninfo.metric_identity)+"\">"+stringutil.isNull(rowninfo.metric_identity)+"</td><td class='over_ellipsis' style='max-width:70px' title=\""+stringutil.isNull(rowninfo.metric_name)+"\">"
            +stringutil.isNull(rowninfo.metric_name)+"</td><td>"+stringutil.isNull(rowninfo.cyclename)+"</td><td class='over_ellipsis' style='max-width:120px' title=\""+stringutil.isNull(rowninfo.script)+"\">"
    ...
    function deleteShow() {
    	...
    	if(checkboxArray.length == 0){
    		...
    		layer.confirm('是否确认删除该批次数据？', {
                ...
                layer.closeAll();
                loadingwait();
                var data = {metricidArray:checkboxArray};
                var url = "/view/class/system/metricmanage/delete/?random=" + Math.random();
