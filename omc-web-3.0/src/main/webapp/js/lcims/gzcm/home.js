...
require(['jquery', "packdata",
      ...
      'echarts/chart/tree'
        ...
        function loadData() {
                //各地市在线用户数
                loadMap("area1","/data/monitor/getOnlineDataHomeMap?chart_name=home_business_online_users&province=gzcm","贵州",
...
//            url = "/js/lcims/nmct/nm_mapdata.json?chart_name=area_online_rate"
           $.getJSON(url+"&random="+Math.random(), function(resultdata) {
                $.get('/js/echarts/util/mapData/rawData/geoJson/gui_zhou_geo.json', function (geoJson) {
                    var myChart = ec.init(document.getElementById(chartid));
                    var features;
                       try {
