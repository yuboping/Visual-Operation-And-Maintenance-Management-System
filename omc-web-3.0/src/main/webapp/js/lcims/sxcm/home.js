...
require(['jquery', "packdata",
      ...
      'echarts/chart/tree'
        ...
        function loadData() {
                //各地市在线用户数
                loadMap("area1","/data/monitor/getOnlineDataHomeMap?chart_name=home_business_online_users&province=sxcm","陕西",
                        "area6","/data/monitor/class/home/Radius");
                //消耗流量
                loadLine("area2","/data/monitor/getDataLine?chart_name=business_consumption_flow_all",2);
                //上行签约速率
                loadRosePie("area3","/data/monitor/getDataRecent?chart_name=business_up_contract_rate_pie");
                //下行签约速率
                loadRosePie("area4","/data/monitor/getDataRecent?chart_name=business_down_contract_rate_pie");
                //最大并发在线用户数
                loadRosePie("area5","/data/monitor/getDataRecent?chart_name=business_max_online_users_pie");
...
//            url = "/js/lcims/nmct/nm_mapdata.json?chart_name=area_online_rate"
           $.getJSON(url+"&random="+Math.random(), function(resultdata) {
                $.get('/js/echarts/util/mapData/rawData/geoJson/shan_xi_1_geo.json', function (geoJson) {
                    var myChart = ec.init(document.getElementById(chartid));
                    var features;
                       try {
