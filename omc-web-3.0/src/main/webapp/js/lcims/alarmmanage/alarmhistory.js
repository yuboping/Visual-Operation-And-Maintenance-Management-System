...
//----------------------------------以下为自定义方法-------------------------------------------------//
    ...
    function loadOptRecord(pageNumber) {
        ...
        $.getJSON("/view/class/system/alarmquery/query?random=" + Math.random(), data, function (result) {
        ...
        function showTable(data,startnum,endnum){
            ...
            for(var i=0;i<=endnum-startnum;i++){
                ...
                +rowninfo.alarmmsg+"</td><td title=\""+rowninfo.first_time+"\">"+rowninfo.first_time+"</td><td title=\""+rowninfo.last_time+"\">"+rowninfo.last_time+"</td><td>"
                +rowninfo.confirm_name+"</td><td title=\""+rowninfo.confirm_time+"\">"+rowninfo.confirm_time+"</td>" +
                    "<td title=\""+stringutil.isNull(rowninfo.clear_time)+"\">"+stringutil.isNull(rowninfo.clear_time)+"</td></tr>";
            }
            ...
            // <td>"+rowninfo.alarm_num+"</td>
            // $("[name=his_detail]").each(function(){
            //     $(this).on('click',function(){
            //         detailShow($(this).attr('id'));
            //     });
            // });
            // +"<a class=\"J_edit pr10\" href=\"#\" name='his_detail' id=\""+rowninfo.alarm_id+"\" >""</a>"+
