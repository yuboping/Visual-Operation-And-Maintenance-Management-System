...
//----------------------------------以下为自定义方法-------------------------------------------------//
    ...
    function loadOptRecord(pageNumber) {
        ...
        var operate_type = $("#operateType").val();
        var operate_name = stringutil.Trim($("#operateName").val());

        loadingwait();
        var data = {
            'start_time': start_time, 'end_time': end_time, 'operate_type': operate_type,
            'operate_name': operate_name, 'pageNumber':pageNumber

        };
        $.getJSON("/view/class/mainttool/operatehis/query?random=" + Math.random(), data, function (result) {
        ...
        function showTable(data,startnum,endnum){
            ...
            for(var i=0;i<=endnum-startnum;i++){
                ...
                rowdata = rowdata + "<tr><td title=\""+rowninfo.operate_name+"\">"+rowninfo.operate_name+"</td><td title=\""+rowninfo.operate_type+"\">"
                +isNull(rowninfo.operate_type)+"</td><td title=\""+rowninfo.operate_desc+"\">"
                +isNull(rowninfo.operate_desc)+"</td><td title=\""+rowninfo.operate_time+"\">"+rowninfo.operate_time+"</td></tr>";
            }
