...
//----------------------------------以下为自定义方法-------------------------------------------------//
    ...
    function loadOptRecord(pageNumber) {
        ...
        var process_name = stringutil.Trim($("#processName").val());
        var error_level = stringutil.Trim($("#errorLevel").val());
        var event_occur_host = stringutil.Trim($("#eventOccurHost").val());

        loadingwait();
        var data = {
            'start_time': start_time, 'end_time': end_time, 'process_name': process_name,
            'error_level': error_level, 'event_occur_host': event_occur_host,'pageNumber': pageNumber

        };
        $.getJSON("/view/class/mainttool/systemlog/query?random=" + Math.random(), data, function (result) {
        ...
        function downloadExcel(){
            ...
            var url = "/view/class/mainttool/systemlog/export?start_time="+start_time+"&end_time="+end_time+"&process_name="+process_name+"&error_level="+error_level+"&event_occur_host="+event_occur_host;
        ...
        function showTable(data,startnum,endnum){
            ...
            for(var i=0;i<=endnum-startnum;i++){
                ...
                +"</td><td title=\""+rowninfo.system_time+"\">"+isNull(rowninfo.system_time)
                +"</td><td title=\""+rowninfo.host_name+"\">"+isNull(rowninfo.host_name)
                +"</td><td title=\""+rowninfo.process_name+"\">"+rowninfo.process_name
                +"</td><td title=\""+rowninfo.error_level+"\">"+rowninfo.error_level
                +"</td><td title=\""+rowninfo.msg_data+"\">"+rowninfo.msg_data
                +"</td></tr>";
            }
