...
require(['jquery', "packdata",
      ...
      'echarts/chart/tree'
        ...
        function loadData() {
                //各地市在线用户数
                loadMap("area1","/data/monitor/getOnlineDataHomeMap?chart_name=area_online_rate","广东",
                ...
                //在线用户数汇总
                loadOnlineLine("area4","/data/monitor/getDataLine?chart_name=BRAS_summary_online_users",4);
                //认证成功率
                loadOnlineLine("area5","/data/monitor/getDataLine?chart_name=allradius_summary_authen_succrate",2);
        		//网管重要指标
        		loadMajorMetric("area8", "/data/monitor/getMajorMetric?chart_name=major_metric");
        };
        
        function loadMajorMetric(chartid, url) {
        	var data = {"metric_names":"request_total,handle_delay_avg,online_staff"};
        	$.getJSON(url+"&random="+Math.random(), data, function(resultdata) {
        		var rowdata = "";
            	$.each(resultdata, function(n, value){
            		rowdata = rowdata + "<br/>" + value;
            	});
            	$("#area10").empty().append(rowdata);
            });
        }
...
//            url = "/js/lcims/nmct/nm_mapdata.json?chart_name=area_online_rate"
           $.getJSON(url+"&random="+Math.random(), function(resultdata) {
                $.get('/js/echarts/util/mapData/rawData/geoJson/guang_dong_geo.json', function (geoJson) {
                    var myChart = ec.init(document.getElementById(chartid));
                    var features;
                    try {
