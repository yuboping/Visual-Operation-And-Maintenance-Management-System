...
//----------------------------------以下为自定义方法-------------------------------------------------//            
    ...
    function showLayer() {
        ...
        $.getJSON("/mainttool/checkLimits?brasip=" + stringutil.Trim($("#brasip").val()) + "&random=" + Math.random(), function(result) {
        	if (result.returncode === "0") {
	        	$.getJSON("/mainttool/checkBasUser?brasip=" + stringutil.Trim($("#brasip").val()) + "&random=" + Math.random(), function(result) {
	                if (result.returncode === "0") {
    ...
    function offlineBatchuser(){
        ...
        if(offlineReason==""){
            layer.tips('请输入下线原因!', '#offlineReason',{ tips: [3, '#EE1A23'] });
        }else{
            lopenByName("#openLoading");
            $.getJSON("/mainttool/offlineBatchuser?brasip=" + $("#brasip").val() + "&offlineReason="+offlineReason
