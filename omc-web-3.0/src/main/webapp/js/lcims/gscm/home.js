...
require(['jquery', "packdata",
      ...
      'echarts/chart/tree'
        ...
        function loadData() {
                //各地市在线用户数
                loadMap("area1","/data/monitor/getOnlineDataHomeMap?chart_name=area_ipv4_active_users","甘肃",
                        "area6","/data/monitor/class/home/Radius");
                //认证总量
                loadLine("area2","/data/monitor/getDataLine?chart_name=device_node_summary_radius_auth_all",2);
                //各失败原因占比
                loadRosePie("area3","/data/monitor/getDataRecent?chart_name=device_node_summary_auth_reject_reason");
                //在线用户数汇总
                loadOnlineLine("area4","/data/monitor/getDataLine?chart_name=online_users",4);
                //认证成功率
                loadOnlineLine("area5","/data/monitor/getDataLine?chart_name=auth_sussces_rate_all",2);
                //网管重要指标
        		loadMajorMetric("homespan", "/data/monitor/gscmMajorMetric?chart_name=major_metric");
        };
        
        function loadMajorMetric(chartid, url) {
        	var data = {"metric_names":"gscm_auth_resp_succ_rate"};
        	$.ajax({
				type: "post",
...
//            url = "/js/lcims/nmct/nm_mapdata.json?chart_name=area_online_rate"
           $.getJSON(url+"&random="+Math.random(), function(resultdata) {
                $.get('/js/echarts/util/mapData/rawData/geoJson/gan_su_geo.json', function (geoJson) {
                    var myChart = ec.init(document.getElementById(chartid));
                    var features;
                    try {
